...
const ReusableResponseForm = ({formTitle,formLabel1,formLabel2,formLabel3,type,description,format,typeFunc,descriptionFunc,formatFunc,handleAddFields,handleRemoveFields}) =>{
    ...
    return(
        <>
            ...
            <input type='text' name='description' placeholder='description' value={description} onChange={ChildToParentDescription}></input><br></br>
            <label>{formLabel3}</label>
            <select value={format} name='format' onChange={ChildToParentFormat}>
                <option>application/json</option>
                <option>application/xml</option>
                <option>application/json</option>
            </select>
            <br></br>
            <button type='button' onClick={()=>handleAddFields()}>Add Response</button>
            <button type='button' onClick={()=>handleRemoveFields()}>Remove Response</button>
        </>
    )
}
